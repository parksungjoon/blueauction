...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		return true;
		/*
		 * if(session.getAttribute("login") != null) { logger.info("SESSION : " +
	...
	/*
	 * private void saveReferer(HttpServletRequest request) { String uri =
	 * request.getRequestURI(); String query = request.getQueryString(); if(query ==
	 * null || query.equalsIgnoreCase("null")) { query = ""; }else { query = "?" +
	 * query; }
	 * 
	 * uri += query;
	 * 
	 * if(request.getMethod().equalsIgnoreCase("get")) { logger.info("원래 접근하려던 경로: "
	 * + uri); // 세션에 저장 request.getSession().setAttribute("referer", uri); } }
	 */
}
